...
bool CT1DecodeDirectFetch::do_fetch_from(int PC){
	if(PC >= 0 && PC < m_code_memory.code_memory_size()){
        ...
        //cout << "Fetching from code memroy at" << PC <<endl;
		return true;
	}
	else
		return false;
...
CT1DecodeDirectFetch::show_instruction(){
	...
	if(m_instruction.OPCODE == MOV3) {
        ...
        unsigned int op2 = (m_instruction.OP2 >>4) & 0xf; // bit masking for negative value
		...
		unsigned int op2 = m_instruction.OP2 & 0xff; // op2 is after bit masking
        ...
        unsigned int op2 = m_instruction.OP2 & 0x0F;
		cout << "MOV2 " << "R" << op2 << ",[R" << m_instruction.OP1 << "]" <<  endl;
	}
		...
		unsigned int op2 = m_instruction.OP2 & 0xf; // op2 is after bit masking
